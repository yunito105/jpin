...
- データと機能クラスを分けることで、、、
  - 同じ業務ロジックがあちこちで重複して使用される
  - どこに業務ロジックが書いてあるのか見通しが悪くなる
　-　画面やDBのCURDに依存してしまう。
本来は、データとロジックを1つのプログラミング単位にまとめるのが、クラスの役割。
Javaが開発された意図と異なる仕組みで作られている。

その為にわかりやすく整理し、変更を楽で安全にするには
- データとロジックを一体にして業務ロジックを整理
- 3層のそれぞれの関心事と業務ロジックの分離を徹底する
- 判断/加工/計算のロジックを一つに
